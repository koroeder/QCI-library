MODULE INTERPOLATION_KEYS
   ...
   REAL(KIND = REAL64), ALLOCATABLE, TARGET :: XPREV(:), GPREV(:)
   REAL(KIND = REAL64), ALLOCATABLE :: EEE(:)
   REAL(KIND = REAL64), ALLOCATABLE :: K_SPRING(:), IMAGE_DIST(:)
   ...
   CONTAINS
      ...
      SUBROUTINE ALLOC_INTCOORDS()
         ...
         ALLOCATE(K_SPRING(NIMAGES+1))
         ALLOCATE(IMAGE_DIST(NIMAGES+1))
         XYZ(:) = 0.0D0
         GGG(:) = 0.0D0
         EEE(:) = 0.0D0
      ...
      SUBROUTINE DEALLOC_INTCOORDS()
         ...
         IF (ALLOCATED(K_SPRING)) DEALLOCATE(K_SPRING)
         IF (ALLOCATED(IMAGE_DIST)) DEALLOCATE(IMAGE_DIST)
      END SUBROUTINE DEALLOC_INTCOORDS

      SUBROUTINE ALLOC_PREVCOORDS()
         USE QCIKEYS, ONLY: NATOMS, NIMAGES
         CALL DEALLOC_PREVCOORDS()
         ALLOCATE(XPREV((3*NATOMS)*(NIMAGES+2)))
         ALLOCATE(GPREV((3*NATOMS)*(NIMAGES+2)))
         XPREV(:) = 0.0D0
         GPREV(:) = 0.0D0
      END SUBROUTINE ALLOC_PREVCOORDS
      
      SUBROUTINE DEALLOC_PREVCOORDS()
         IF (ALLOCATED(XPREV)) DEALLOCATE(XPREV)
         IF (ALLOCATED(GPREV)) DEALLOCATE(GPREV)
      END SUBROUTINE DEALLOC_PREVCOORDS
      ...
      SUBROUTINE ALLOC_STEPTAKING()
         ...
         ALLOCATE(GTMP(3*NATOMS*NIMAGES))          !< direction for the steepest descent
