module perm_setup
   ...
   contains
      subroutine alloc_perm()
         ...
         call dealloc_perm()
         allocate(groupactive(npermgroup))
      ...
      subroutine setup_perm()
         use qcikeys
         use QCIFILEHANDLER, only: file_exists, GETUNIT

         implicit none
         integer :: funit
         integer :: ndummy, j1, j2, j3

         if (.not.file_exists("perm.allow")) then
            write(*,*) "In setup_perm, perm.allow file not found - terminating"
            stop
         end if
         ...
         do j1=1,npermgroup
            ...
            if (nsets(j1).gt.maxnsets) then
               write(*,*) "perm_Setup> number of secondary sets exceeds limit"
               stop
            end if
            if (ndummy+npermsize(j1)-1.gt.natoms) then              
               write(*,*) " perm_Setup> number of atoms to be permuted in all groups is > 3*number of atoms"
               stop
            endif
         ...
         ingroup(1:natoms) = 0 !is this the correct default initialisation?
