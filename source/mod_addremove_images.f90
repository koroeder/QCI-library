MODULE ADDREMOVE_IMAGES
   ...
   CONTAINS
      ...
      SUBROUTINE ADD_IMAGE(IDXMAX, ETOTAL, RMS)
         ...
         USE INTERPOLATION_KEYS, ONLY: ALLOC_STEPTAKING
         IMPLICIT NONE
         ...
         DO J1=1,MUPDATE
            ...
            END IF
            SEARCHSTEP(OFFSET1+1:OFFSET2) = TEMPSTEP(J1,3*NATOMS*(MIN(IDXMAX,NIMAGES-1))+1:3*NATOMS*(MIN(IDXMAX,NIMAGES-1)))
            GDIF(OFFSET1+1:OFFSET2) = TEMPGDIF(J1,3*NATOMS*(MIN(IDXMAX,NIMAGES-1))+1:3*NATOMS*(MIN(IDXMAX,NIMAGES-1)))
         END DO
