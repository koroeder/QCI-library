   ...
   CONTAINS
      ...
      SUBROUTINE DETECT_LINEAR()   
         ...
         INTEGER :: NGROUPS
          ...
          ! ========== NEW: Check for geometric rigidity ==========
         ...
         REAL(KIND=REAL64) :: ANGLE_TOLERANCE = 0.1745329251994329 !0.08726646259971647 !5.0D0 degrees
         ...
         INTEGER :: EXTERNAL_ATOMS(NATOMS)     ! The single backbone atom per group
      ...
      DO J1 = 1, NGROUPS
         ...
         DO J2 = 1, NINGROUP_TEMP(J1)
            ...
            DO J3 = 1, N_BONDS_PER_ATOM(ATOM)
               ...
               IF (NEIGHBOR_ATOM < 1 .OR. NEIGHBOR_ATOM > NATOMS) THEN
                  PRINT *, "ERROR: Invalid neighbor index ", NEIGHBOR_ATOM
                  CYCLE
               END IF

               ! Bond crosses the group boundary
               IF (CURRENTGROUP(NEIGHBOR_ATOM) /= CURRENTGROUP(ATOM)) THEN

                  ! Record only DISTINCT external atoms
                  IF (.NOT. IS_IN_LIST(NEIGHBOR_ATOM, EXTERNAL_ANCHORS, &
                                       N_EXTERNAL_ANCHORS)) THEN
         ...
         IF (IS_VALID_GROUP) THEN
            ...
            EXTERNAL_ATOMS(GROUPID) = BACKBONE_ATOM
            ...
            PRINT *, "Valid Linear Group ", GROUPID, ": ", NINGROUP(GROUPID), " atoms"
            PRINT *, "  Backbone atom (attachment point): ", BACKBONE_ATOM
            PRINT *, "  Group atoms: ", (LINEAR_GROUPS(GROUPID, J2), &
                                         J2=1, NINGROUP(GROUPID))
            
            ! Show external connections from backbone atom
            TEMP_EXTERNAL_COUNT = 0
            DO J3 = 1, N_BONDS_PER_ATOM(BACKBONE_ATOM) !NCONPERATOM(BACKBONE_ATOM)
               !NEIGHBOR_ATOM = CONLIST(BACKBONE_ATOM, J3)
               NEIGHBOR_ATOM = BONDS_PER_ATOM_LIST(BACKBONE_ATOM, J3)
               IF (ATOM2LINGROUP(NEIGHBOR_ATOM) /= GROUPID) THEN
                  TEMP_EXTERNAL_COUNT = TEMP_EXTERNAL_COUNT + 1
                  PRINT *, "    Connected to external atom: ", NEIGHBOR_ATOM
               END IF
            END DO
         ELSE
