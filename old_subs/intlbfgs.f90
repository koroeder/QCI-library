...
USE KEY, ONLY : FREEZENODEST, FREEZETOL, MAXINTBFGS, CONVR, ATOMSTORES, &
     ...
     & QCIFREEZELIST, QCIFREEZEN, QCIFREEZEATOMS, MAXGRADCOMP, QCILINEARCUT

USE BOX_DERIVATIVES, ONLY : RESTOREMINIMAGE
USE COMMONS, ONLY: NATOMS, DEBUG, ZSYM, PARAM1, PARAM2, PARAM3
USE MODCHARMM, ONLY : CHRMMT, CHECKCHIRALT
USE CHIRALITY
...
!  PRINT '(A)',' intlbfgs> on entry QSTART:'
!  PRINT '(3G20.10)',QSTART(1:3*NATOMS)
!  PRINT '(A)',' intlbfgs> on entry QFINISH:'
!  PRINT '(3G20.10)',QFINISH(1:3*NATOMS)
...
653 CONTINUE
   ...
   DO J1=1,NATOMS
      READ(LUNIT,*) SDUMMY
!     WRITE(*,'(A,I6,A2,3G20.10)') 'J1,sd,xd,yd,zd=',J1,SDUMMY
   ENDDO
   INTIMAGE=INTIMAGE+1
   GOTO 653
654 CONTINUE
...
IF (QCIIMAGEDENST) THEN
   ...
   INTIMAGE=MAX(MIN(IMDENSITY*LDISTANCE,1.0D0*MAXINTIMAGE),1.0D0)
...
! PRINT '(3G20.10)',QFINISH(1:3*NATOMS)
  DO J2=1,NATOMS
   ...
   ENDDO
!  PRINT '(A)',' intlbfgs> QCI after minimum image vector QFINISH:'
!  PRINT '(3G20.10)',QFINISH(1:3*NATOMS)
ENDIF
!
! Initialise XYZ
...
ENDIF
!
! Freeze atoms in QCIFREEZEATOMS list and use linear interpolation for them
! We may no have the INTFREEZE keyword set, so need to turn on INTFREEZET in case it is false.
!
IF (QCIFREEZELIST) THEN
   INTFREEZET=.TRUE.
   DO J1=1,QCIFREEZEN
      J2=QCIFREEZEATOMS(J1)
      IF (INTFROZEN(J2)) THEN
!        WRITE(*,'(A,I6,A,F12.6,A,I6)') ' intlbfgs> atom ',J2,' was already frozen'
      ELSE 
         NQCIFREEZE=NQCIFREEZE+1
      ENDIF
      INTFROZEN(J2)=.TRUE.
      IF (DEBUG) WRITE(*,'(A,I6,A,F12.6,A,I6)') ' intlbfgs> freezing atom ',J2,' and using linear interpolation'
   ENDDO
ENDIF
!
! Freeze atoms in QCILINEARATOMS list and use linear interpolation for them
!
! IF (QCILINEARLIST) THEN
!    DO J1=1,QCILINEARN
!       J2=QCILINEARATOMS(J1)
!       IF (INTFROZEN(J2)) THEN
!          WRITE(*,'(A,I6,A,F12.6,A,I6)') ' intlbfgs> atom ',J2,' was already frozen'
!       ELSE 
!          NQCIFREEZE=NQCIFREEZE+1
...
! ENDIF
!
! Add all the atoms in the QILINEARATOMS list with linear interpolation, but don't freeze them
!
! IF (QCILINEARLIST) THEN
!    CALL DOADDATOM(NCONSTRAINT,NEWATOM,IMGFREEZE,INTIMAGE,XYZ,EEE,GGG,TURNONORDER,NITERDONE,NACTIVE,AABACK,BACKDONE)  
!    LUNIT=GETUNIT()
!    OPEN(UNIT=LUNIT,FILE='added.xyz',STATUS='replace')
!    DO J2=1,INTIMAGE+2
!       WRITE(LUNIT,'(i4/)') NATOMS
!       DO J3=1,NATOMS
!             WRITE(LUNIT,'(A5,1X,3F20.10)') 'LA   ',XYZ((J2-1)*3*NATOMS+3*(J3-1)+1),XYZ((J2-1)*3*NATOMS+3*(J3-1)+2), &
...
! ENDIF
!
! For QCILINEARCUT positive threshold use a list created on-the-fly in every QCI call.
! For a list read from disc we only use that list once for the first call and QCILINEARCUT is < 0
! These atoms will all be made active in the first call to DOADDATOM
!
IF (QCILINEARCUT.GT.0.0D0) THEN
   IF (.NOT.ALLOCATED(QCILINEARATOMS)) ALLOCATE(QCILINEARATOMS(NATOMS))
   QCILINEARLIST=.TRUE. ! this logical is set to false after every QCI initialisation, so need to reset true
   QCILINEARN=0
   DUMMY=QCILINEARCUT**2
   DO J1=1,NATOMS
      DF=(XYZ(3*(J1-1)+1)-XYZ((INTIMAGE+1)*3*NATOMS+3*(J1-1)+1))**2 &
  &     +(XYZ(3*(J1-1)+2)-XYZ((INTIMAGE+1)*3*NATOMS+3*(J1-1)+2))**2 &
  &     +(XYZ(3*(J1-1)+3)-XYZ((INTIMAGE+1)*3*NATOMS+3*(J1-1)+3))**2
      IF (DF.LT.DUMMY) THEN
         QCILINEARN=QCILINEARN+1
         QCILINEARATOMS(QCILINEARN)=J1
         IF (DEBUG) WRITE(*,'(A,I6,A,F12.6,A,I6)') &
  &            ' intlbfgs> atom ',J1,' moves less than QCILINEAR threshold: dist^2=',DF,' total=',QCILINEARN
      ENDIF
   ENDDO
   WRITE(*,'(A,I6,A,F12.6,A,I6)') ' intlbfgs> Total number of atoms moving less than QCILINEAR threshold=',QCILINEARN
ENDIF
...
ELSE
   ...
   GOTO 678
ENDIF
!  PRINT '(A)',' intlbfgs> QCI here A last frame is:'
!  PRINT '(3G20.10)',XYZ((3*NATOMS)*(INTIMAGE+1)+1:(3*NATOMS)*(INTIMAGE+2))
...
IF (QCIADDREP.GT.0) THEN ! extra repulsions - do not use
   CALL CONGRAD3(NMAXINT,NMININT,ETOTAL,XYZ,GGG,EEE,IMGFREEZE,RMS)
ELSEIF (CHECKCONINT) THEN ! include check for internal minima in constraints - not the default
   CALL CONGRAD2(NMAXINT,NMININT,ETOTAL,XYZ,GGG,EEE,IMGFREEZE,RMS)
ELSE ! the default
   CALL CONGRAD(NMAXINT,NMININT,ETOTAL,XYZ,GGG,EEE,IMGFREEZE,RMS)
ENDIF
IF (MAXGRADCOMP.GT.0.0D0) CALL SCALEGRAD(D,G,RMS,MAXGRADCOMP) ! Reduce gradient components to maximum allowed if MAXGRADCOMP > 0
EOLD=ETOTAL
GLAST(1:D)=G(1:D)
XSAVE(1:D)=X(1:D)
...
! GOTO 642  ! DJW skip chiral check now DEBUG

   IF (CHECKCHIRALT) CALL DOCHICHECK(NEIGHBOUR_COORDS,CENTRE_COORDS,XYZ,NUM_CHIRAL_CENTRES)

! 642 CONTINUE ! DJW jump here to skip chiral check

!  PRINT '(A,I6)','NPERMGROUP=',NPERMGROUP
   NDUMMY=1
   DO J1=1,NPERMGROUP
!     PRINT '(A,I6,L5,I6)','J1,GROUPACTIVE,NPERMSIZE=',J1,GROUPACTIVE(J1),NPERMSIZE(J1)
...
!       IF (DEBUG) WRITE(*,'(A)') ' intlbfgs> dump state before lopermdist index -2'
!       IF (DEBUG) CALL INTRWG2(NACTIVE,-2,INTIMAGE,XYZ,TURNONORDER,NCONOFF)
   ...
   np: DO J1=1,NPERMGROUP
      IF (.NOT.GROUPACTIVE(J1)) THEN
         ...
         CYCLE np
      ENDIF
!        WRITE(*,'(A,I6,A,I6)') ' intlbfgs> Doing group ',J1
      DO J3=1,INTIMAGE 
!        WRITE(*,'(A,I6,A,I6,A,I6)') ' intlbfgs> Doing group ',J1,' align image ',J3+1,' with ',J3
!            coordinates for intervening image J3-1, which is the starting endpoint for J3=1
...
! !          CALL QCIOPTPERM(COORDSB,COORDSA,NATOMS,J1,PERM,STARTGROUP,J3)
! !          IF (.NOT.IDENTITY) THEN
! !             DO J2=1,NATOMS
! ! !           IF (PERM(J2).NE.J2) WRITE(*,'(4(A,I6))') 'intlbfgs> image ',J3+1,&
!      &          ' qcipermopt would move atom ',J2,' to position ',PERM(J2), &  
! ! !  &   ' group ',J1  
! !                IF (PERM(J2).NE.J2) WRITE(*,'(4(A,I6))') 'intlbfgs> image ',J3+1,&
!      &          ' qcipermopt move atom ',J2,' to position ',PERM(J2), &
! !    &   ' group ',J1  
...
! !        IF (QCILPDCON) THEN
! !           CALL LOPERMDISTCON(COORDSB,COORDSA,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,DISTANCES,DIST2,J1,NMOVES,PERMS)
! !        ELSE
!             CALL LOPERMDIST(COORDSB,COORDSA,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,TWOD,DISTANCES,DIST2,.FALSE.,RMATBEST,J1,&
!     &            NMOVES,PERMS)
! !        ENDIF
!          PBS=PBETTER
!          PRINT '(A,I6,A,2I6,A,I6,A,2G20.10)','intlbfgs> group ',J1,' alignment of images ',1,J3+1,' moves=',NMOVES,  &
!        WRITE(*,'(A,I6,A,I6,A,I6)') ' intlbfgs> Doing group ',J1,' align image ',J3+1,' with ',J3
!   &            ' permutations, distance with/without permutations=',PDISTANCE,NOPDISTANCE
!          coordinates for intervening image J3, which is image J3+1 including starting endpoint
!          COORDSA(1:3*NATOMS)=XYZ(3*NATOMS*J3+1:3*NATOMS*(J3+1)) 
! !        WRITE(*,'(A,I6,A,I6,A)') ' intlbfgs> Doing group ',J1,' image ',J3+1,' with finish'
! !        IF (QCILPDCON) THEN
! !           CALL LOPERMDISTCON(COORDSC,COORDSA,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,DISTANCEF,DIST2,J1,NMOVEF,PERMF)
! !        ELSE
!             CALL LOPERMDIST(COORDSC,COORDSA,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,TWOD,DISTANCEF,DIST2,.FALSE.,RMATBEST,J1,&
!   &             NMOVEF,PERMF)
! !        ENDIF
!          PBF=PBETTER
!          PRINT '(A,I6,A,2I6,A,I6,A,2G20.10)','intlbfgs> group ',J1,' alignment of images ',INTIMAGE+2,J3+1,' moves=',NMOVEF,  &
!   &            ' permutations, distance with/without permutations=',PDISTANCE,NOPDISTANCE     
!          IF ((NMOVES.GT.0).OR.(NMOVEF.GT.0)) THEN
!             IF (PBS.AND.PBF) THEN
!                WRITE(*,'(A,I6,A,I6,A,2G20.10,A,2I6,A)') ' intlbfgs> image ',J3+1,' group ',J1,' start and finish distances=', &
!    &                                           DISTANCES,DISTANCEF,' permutations=',NMOVES, NMOVEF,' better overall for both'
!             ELSEIF (PBS) THEN
!                WRITE(*,'(A,I6,A,I6,A,2G20.10,A,2I6,A)') ' intlbfgs> image ',J3+1,' group ',J1,' start and finish distances=', &
!    &                                           DISTANCES,DISTANCEF,' permutations=',NMOVES, NMOVEF,' better overall for start only'
!             ELSEIF (PBF) THEN
!                WRITE(*,'(A,I6,A,I6,A,2G20.10,A,2I6,A)') ' intlbfgs> image ',J3+1,' group ',J1,' start and finish distances=', &
!    &               DISTANCES,DISTANCEF,' permutations=',NMOVES, NMOVEF,' better overall for finish only'  
!             ELSE
!                WRITE(*,'(A,I6,A,I6,A,2G20.10,A,2I6,A)') ' intlbfgs> image ',J3+1,' group ',J1,' start and finish distances=', &
!    &               DISTANCES,DISTANCEF,' permutations=',NMOVES, NMOVEF,' worse overall for both'
!             ENDIF
!          ENDIF
!
...
!          IF ((NMOVES.GT.0).AND.(NMOVES.EQ.NMOVEF).AND.PBS.AND.PBF) THEN
!             COORDSA(1:3*NATOMS)=XYZ(3*NATOMS*J3+1:3*NATOMS*(J3+1)) 
! !           DO J2=1,NATOMS ! don't need to look at all atoms, just those in group J1
!             DO J4=1,NPERMSIZE(J1)
!                J2=PERMGROUP(NPSTART+J4-1)
! !              PRINT '(A,2I6)','J2,PERMS(J2)=',J2,PERMS(J2)
!                IF (PERMS(J2).NE.J2) THEN
! !                 PRINT '(A,3I6)','J2,PERMS(J2),PERMF(J2)=',J2,PERMS(J2),PERMF(J2)
!                   IF (PERMF(J2).EQ.PERMS(J2)) THEN
! !                    WRITE(*,'(A,I6,A,I6,A,I6)') ' intlbfgs> image ',J3+1, &
! !  &                 ' consistent non-identity lopermdist permutation for start and finish, move atom ',&
!    &                 PERMS(J2),' to position ',J2   
!                  WRITE(*,'(A,I6,A,I6,A,I6)') ' intlbfgs> image ',J3+1, &
!    &           ' consistent non-identity lopermdist permutation for start and finish, would move atom ',&
!    &           PERMS(J2),' to position ',J2  
...
!        WRITE(*,'(A,I6,A,I6,A,I6)') ' intlbfgs> Doing group ',J1,' align image ',J3+1,' with ',J3+2
!
...
!       IF (DEBUG) WRITE(*,'(A)') ' intlbfgs> dump state after lopermdist index -1'
...
!!                     This line is not in make_conpot, where we want all possible repulsions.
!!                     IF (.NOT.ATOMACTIVE(J2)) CYCLE myrep2
!!                     DMIN=1.0D100
...
!!                  ENDDO myrep2
!!               ENDDO
!!               WRITE(*,'(A,I6,A)') ' intlbfgs> Now it looks like there are ',NREPULSIVE,&
!!    &              ' possible repulsions before adding new atom'
!!!!!!!!!!!!!!!DEBUG DJW !!!!!!!!!!!
...
!        IF (DEBUG) WRITE(*,'(A,G20.10,A,G20.10)') ' intlbfgs> Mean image separation=',DUMMY2/(INTIMAGE+1),&
! &          ' per active atom=',DUMMY2/((INTIMAGE+1)*NACTIVE)
...
!        IF (DEBUG) WRITE(*,'(A,G20.10,A,2I6)')' intlbfgs> largest image separation is ', &
! &                                                  DMAX,' for images ',JMAX,JMAX+1
!        IF (DEBUG) WRITE(*,'(A,G20.10,A,G20.10)') ' intlbfgs> Mean image separation=',DUMMY2/(INTIMAGE+1),' per active atom=',&
! &          DUMMY2/((INTIMAGE+1)*NACTIVE)
...
!
! Now we get a new energy and gradient.
!
   IF (.NOT.SWITCHED) THEN
...
!      LASTGOODE=ETOTAL
   ENDIF
!
! Reduce gradient components to maximum allowed if MAXGRADCOMP > 0
!
   IF (MAXGRADCOMP.GT.0.0D0) CALL SCALEGRAD(D,G,RMS,MAXGRADCOMP)

   EXITSTATUS=0
   INTDGUESS=DIAG(1) ! should be ok for subsequent runs of the same system DJW
!  IF ((.NOT.SWITCHED).AND.(MAXRMS<=INTRMSTOL).AND.(NITERDONE>1).AND.(CONVERGECONTEST.LT.MAXCONE).AND.&
! &    (CONVERGEREPTEST.LT.MAXCONE)) EXITSTATUS=1 
!  PRINT '(A,L5,2I10)','intlbfgs> SWITCHED,NITERDONE,EXITSTATUS=',SWITCHED,NITERDONE,EXITSTATUS
...
!  PRINT '(A,2G20.10)','intlbfgs> FCONTEST,INTRMSTOL=',FCONTEST,INTRMSTOL
!  PRINT '(A,2G20.10)','intlbfgs> FREPTEST,INTRMSTOL=',FREPTEST,INTRMSTOL
!  PRINT '(A,2G20.10)','intlbfgs> CONVERGECONTEST,MAXCONE=',CONVERGECONTEST,MAXCONE
!  PRINT '(A,2G20.10)','intlbfgs> CONVERGEREPTEST,MAXCONE=',CONVERGEREPTEST,MAXCONE
!  PRINT '(A,I8)','intlbfgs> EXITSTATUS=',EXITSTATUS
!  IF (SWITCHED.AND.(MAXRMS<=CONVR).AND.NITERDONE>1) EXITSTATUS=1  ! this test is not a good idea for DNEBPATH with huge CONVR
!  PRINT '(A,2G20.10)','intlbfgs> MAXRMS,CONVR=',MAXRMS,CONVR
!  PRINT '(A,I8)','intlbfgs> EXITSTATUS=',EXITSTATUS
!  PRINT '(A,2I8)','intlbfgs> NITERDONE,NSTEPSMAX=',NITERDONE,NSTEPSMAX
...
!           SETS(1:NATOMS,1:NPERMGROUP,1:MAXNSETS)=SETSBACK(1:NATOMS,1:NPERMGROUP,1:MAXNSETS)
!        ENDIF
!        RETURN ! this will cause the program to stop - going to EXIT should allow us to continue instead.
...
DO J2=1,INTIMAGE+2
   ...
   ELSE
      ...
      WRITE(LUNIT,'(A,9G25.15)') 'Lattice="',LOCALCELL(1:9)
      CALL FRAC2CART(XYZTEMP,XYZFRAME)
!     PRINT '(A,I8,A,6G20.10)','frame ',J2,' box: ',BOX_PARAMS(1:6)
!     PRINT '(A,I8)','fractional XYZ:'
!     PRINT '(3G20.10)',XYZFRAME(1:3*NATOMS)
!     PRINT '(A,I8)','cartesian XYZ:'
!     PRINT '(3G20.10)',XYZTEMP(1:3*NATOMS)
...
!                 ELSEIF (QCILINEARLIST.AND.(.NOT.INLIST(CONI(J1)))) THEN ! changed to allow inactive atoms outside linearlist
                  ELSE
...
!                 ELSEIF (QCILINEARLIST.AND.(.NOT.INLIST(CONJ(J1)))) THEN ! changed to allow inactive atoms outside linearlist
                  ELSE
...
!     IF (QCILINEARLIST) WRITE(*,'(A,L5)') ' intlbfgs> INLIST=',INLIST(NEWATOM)
...
!
!           PRINT *,'NREPULSIVEFIX,NCONSTRAINTFIX=',NREPULSIVEFIX,NCONSTRAINTFIX
!             DO J1=1,NREPULSIVEFIX
!                IF (INTFROZEN(REPIFIX(J1)).AND.INTFROZEN(REPJFIX(J1))) CYCLE
!                IF (ATOMACTIVE(REPIFIX(J1)).AND.ATOMACTIVE(REPJFIX(J1))) THEN
!                   DO J3=1,NCONSTRAINTFIX
...
! !           WRITE(*,'(A,9G25.15)') ' intlbfgs> VEC1,2,3=',VEC1(1:3),VEC2(1:3),VEC3(1:3)
! !           WRITE(*,'(A,6I6)') ' intlbfgs> N1,N2,N3,Bestpreserved N1,N2,N3=',N1,N2,N3, &
! ! &              BESTPRESERVEDN(N1),BESTPRESERVEDN(N2),BESTPRESERVEDN(N3)
! 
! !
! ! Alternative analytical solution from intersection of three spheres by trilateration not available -
! ! we haven't saved the distances, just the differences in the endpoints
! !
!          ENDDO
! 
!          CALL CHECKREP(INTIMAGE,XYZ,(3*NATOMS),NNREPSAVE,NREPSAVE+1) ! set up repulsive neighbour list
...
!
  ...
  &             CONJ(NCONSTRAINT),' values are ',DS,DF,' fraction=',2*ABS(DS-DF)/(DS+DF), &
  &            ' # bond constraints=',NCONSTRAINT
!      IF (NCONSTRAINT.EQ.53) WRITE(*,'(A,2I6,A,2F12.2,A,F12.4,A,I8)') ' intlbfgs> constrain distance for ',CONI(NCONSTRAINT), &
!  &             CONJ(NCONSTRAINT),' values are ',DS,DF,' fraction=',2*ABS(DS-DF)/(DS+DF), &
!  &            ' # bond constraints=',NCONSTRAINT
   ENDDO
   QCIBONDS=NCONSTRAINT
...
DO J3=1,INTIMAGE+1 
    ...
    DO J4=1,NACTIVE
       J2=TURNONORDER(J4)
!      XYZ(3*NATOMS*J3+3*(J2-1)+1:3*NATOMS*J3+3*(J2-1)+3)=COORDSB(3*(J4-1)+1:3*(J4-1)+3)  
!      WRITE(LUNIT2,'(A,3G20.10)') 'LA ',XYZ(3*NATOMS*(J3  )+3*(J2-1)+1:3*NATOMS*(J3  )+3*(J2-1)+3)
...
!
! Subroutine to scale down excessive gradient components to maximum value of MAXGRADCOMP
!
SUBROUTINE SCALEGRAD(D,G,RMS,MAXGRADCOMP)
   IMPLICIT NONE
   INTEGER, INTENT(IN) :: D
   DOUBLE PRECISION, INTENT(IN) :: MAXGRADCOMP
   DOUBLE PRECISION, INTENT(INOUT) :: G(D), RMS
   DOUBLE PRECISION RMSSAVE
   INTEGER J1
   RMSSAVE=RMS
   RMS=0.0D0
   DO J1=1,D
   ...
   ENDDO
   RMS=SQRT(RMS/D)
   PRINT '(A,3G20.10)', 'scalegrad> RMS, RMSSAVE, MAXGRADCOMP=',RMS,RMSSAVE,MAXGRADCOMP
